...
function part1(input) {
    ...
    let path = [new Robot(input,3, 20, 20).run(ctx)]
    ...
    while(i<20) {
        ...
        let hit = 0;
        for(let w = 0; w < last.ways.length; w ++) {

            const robot = new Robot(last.program, last.ways[w], last.x, last.y, last.steps, last.history).run(ctx)
